...
data ReqBodyContentType = ReqBodyPlainText | ReqBodyMultipart
...
data Req ftype = Req
  ...
  --
  -- @• PlainText expected in list '[MyFancyContentType]@
  }
  deriving (Data, Eq, Show, Typeable)
...
defReq :: Req ftype
defReq = Req defUrl "GET" [] Nothing Nothing (FunctionName []) ReqBodyPlainText
...
instance (Elem PlainText list, HasForeignType lang ftype a, ReflectMethod method)
...
instance (ct ~ PlainText, HasForeignType lang ftype a, ReflectMethod method)
...
instance (Elem PlainText list, HasForeignType lang ftype a, HasForeign lang ftype api)
